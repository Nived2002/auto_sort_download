...
dest_dir_sfx = r"C:\\Categories\\SFX"
dest_dir_music = r"C:\\Categories\\Music"
dest_dir_video = r"C:\\Categories\\Videos"
dest_dir_image = r"C:\\Categories\\Images"
dest_dir_documents = r"C:\\Categories\\Documents"
...
class MoverHandler(FileSystemEventHandler):
    ...
    def check_video_files(self, entry, name):  #  Video Files
        for video_extension in video_extensions:
            if name.endswith(video_extension) or name.endswith(video_extension.upper()):
                move_file(dest_dir_video, entry, name)
                logging.info(f"Moved video file: {name}")

    def check_image_files(self, entry, name):  #  Image Files
        for image_extension in image_extensions:
            if name.endswith(image_extension) or name.endswith(image_extension.upper()):
                move_file(dest_dir_image, entry, name)
                logging.info(f"Moved image file: {name}")

    def check_document_files(self, entry, name):  # Document Files
